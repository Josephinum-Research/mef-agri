...
from ...utils.misc import set_attributes, PixelUnits
...
class Field(GeoRaster):
    ...
    with same order containing the uniqu zone ids (from field name and 
    zone name).

    Zone element in the `zones`-dictionary

    * zname - name of the zone
    * gcs - geo coordinates representing the zone - (2, n) numpy.ndarray
    * lat - mean latitudeo of the zone (see `_get_mean_latitude_wgs84()`)
